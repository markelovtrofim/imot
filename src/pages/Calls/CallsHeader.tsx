...
const CallsHeader: FC<CallsHeaderPropsType> = React.memo((
  ...
  const [sortArrowType, setSortArrowType] = useState<boolean>(true);
  ...
  const [MWButtonLoading, setMWButtonLoading] = useState<boolean>(true);
  ...
  async function updateSortDesc() {
    setSortArrowType(!sortArrowType);
    dispatch(callsSlice.actions.setSort(sort));
    dispatch(callsSlice.actions.zeroingSkip(null));
    dispatch(callsSlice.actions.setEmptyState({ leaveBundles: 0 }))
    dispatch(getBaseCallsData({ sort: sort, sortDesc: !sortArrowType }));
  }
    ...
    <div className={classes.callsHeader}>
      <div className={classes.callsTitle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <div className="sort-block-component">
            <ContainedSelect
              width={"150px"}
              onSelectChange={(event) => {
                if (event.value === "grade") {
                  ...
                  if (event.value === "duration") {
                    dispatch(getBaseCallsData({ sort: event.value, sortDesc: sortArrowType }))
                  } else if (event.value === "date") {
                    dispatch(getBaseCallsData({ sortDesc: sortArrowType }))
                  }
                }
              }}
              options={sortArray}
            ...
            />
            <div className="select-type-sort" onClick={updateSortDesc}>
              {
                sortArrowType
